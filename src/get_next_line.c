...
/*   Updated: 2023/01/03 16:05:25 by alixavezou       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_print_line(char *buffer_save)
{
	...
	while (buffer_save[i] && buffer_save[i] != '\n')
		i++;
	line = _substr(buffer_save, 0, i + 1);
	if (!line)
	{
		free(line);
		return (NULL);
...
char	*ft_get_buffer_save(char *buffer_save)
{
	...
	while (buffer_save[i])
	{
		...
		{
			i += 1;
			new_buffer_save = _substr(buffer_save, i, (strlen - i) + 1);
			break ;
		}
...
char	*ft_read(int fd, char *buffer_read)
{
	...
	while (ret > 0)
	{
		...
		buffer_read[ret] = '\0';
		if (buffer_save == NULL)
			buffer_save = _strdup(buffer_read);
		else
			buffer_save = _strjoin(buffer_save, buffer_read);
	}
